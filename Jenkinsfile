...
dockerTemplate{
   clientsNode {
    ws{
      ...
      if (utils.isCI()){
        def snapshotImageName = "fabric8/sonarqube:SNAPSHOT-${env.BRANCH_NAME}-${env.BUILD_NUMBER}"
        container('docker'){
        ...
        stage('notify'){
            ...
            def message = "@${changeAuthor} snapshot Sonarqube image is available for testing.  `docker pull ${snapshotImageName}`"
            container('docker'){
        ...
        def imageName = "fabric8/sonarqube:${v}"
